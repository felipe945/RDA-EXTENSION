...
export function buildInviteEmail(opts: {
  ...
  const subject = `${first} invited you to FanMas`;

  const extensionUrl = new URL("/settings/extension", opts.inviteUrl).toString();

  const text = [
    `${first} invited you to FanMas as a ${roleLabel} — the dashboard the team uses for leads, outreach, and booking.`,
    ``,
  ...
  <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:8px;padding:32px 28px;color:#0D0D0E;">
    <div style="font-weight:800;font-size:16px;letter-spacing:-0.02em;margin:0 0 20px;">FanMas</div>
    <h1 style="font-size:20px;font-weight:700;margin:0 0 10px;color:#0D0D0E;">${h.first} invited you to the team</h1>
    <p style="margin:0 0 18px;color:#3F3F3D;font-size:14px;line-height:1.6;">You&rsquo;ve been added as a <strong>${h.role}</strong> on FanMas &mdash; the dashboard the team uses for leads, outreach, and booking.</p>
